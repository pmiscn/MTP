...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result := Result + TMExp.doExpression(exp, true, aData, aDataParser,aParams);
        Inc(p, EXEndLLength);
        estart    := p - p1 + 1;
        isInStart := false;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // 把数据传给下一级
        nvalue := nvalue + ParseNode(aNode[i], aData, aDataParser, aPath, aParams, aLoopVar, aLoopValue);
      end;
      // 吧后面的加上
  ...
  begin
    ...
    begin
      ...
      if aDataParser.TryDataByPath(aData, aNode.DataPath, Data) then
      begin
      end
      else
    ...
    Result := TMExp.doExpression(exp, true, Data, aDataParser,aParams);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // outputdebugstring(Pchar(ajs.tostring));
        r := TMExp.doExpression(r, true, ajs, aDataParser,aParams);
        if r <> 'True' then
          Continue;
      end;
  ...
  var
    ...
    begin
      ...
      begin
        ...
        r := TMExp.doExpression(exp, true, Data, aDataParser,aParams);
        if r <> 'True' then
          exit;
      end;
  ...
  begin
    ...
    r      := TMExp.doExpression(r, true, aData, aDataParser,aParams);
    if r = 'True' then
    begin
      ...
      begin
        ...
        begin
          if TMExp.doExpression(replaceVarValue(nd.Condition, aLoopVar, aLoopValue), true, aData, aDataParser,aParams) = 'True'
          then
          begin
