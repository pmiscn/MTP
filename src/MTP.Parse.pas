...
var
  ...
  begin
    ...
    if not TryStrToInt(aNode.Range.RStart, fstart) then
    begin
      startstr := TMExp.doExpression(aNode.Range.RStart, true, aData, aDataParser, aParams, aRootData);
      fstart   := strtoint(startstr);
    end;
    if fstart < 0 then
      fstart := 0;
    if not TryStrToInt(aNode.Range.REnd, fend) then
    begin
      endstr := TMExp.doExpression(aNode.Range.REnd, true, aData, aDataParser, aParams, aRootData);
      fend   := strtoint(endstr);
    end;
